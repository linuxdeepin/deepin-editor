...
step_2_configure_cmake() {
    ...
    # Optional user-local dev prefix (e.g. when -dev packages are not installed
    # system-wide). See autotests/README.md for reproducible build instructions.
    if [ -f "$HOME/ut-deps/env.sh" ]; then
        print_info "Sourcing user-local dev prefix: $HOME/ut-deps/env.sh"
        # shellcheck disable=SC1091
        source "$HOME/ut-deps/env.sh"
    fi

    # Headless Qt for both gtest_discover_tests (build-time) and test execution.
