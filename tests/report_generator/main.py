...
class TestReportGenerator:
    ...
    def __init__(self, build_dir: str, report_dir: str, project_root: str, results_dir: str | None = None):
        ...
        self.results_dir = Path(results_dir) if results_dir else self.project_root / "tests" / ".results"
        self.test_data = {}
...
if __name__ == "__main__":
    ...
    cli.add_argument("--results-dir", default=None,
                     help="gtest XML results dir (default: <project-root>/tests/.results)")
    cli.add_argument("--session", default=None,
                     help="session JSON path (default: <project-root>/tests/.ut-session.json)")
    args = cli.parse_args()

    build_dir = Path(args.build_dir)
    report_dir = Path(args.report_dir)
    project_root = Path(args.project_root)
    results_dir = Path(args.results_dir) if args.results_dir else project_root / "tests" / ".results"
